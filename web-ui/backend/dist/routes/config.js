...
router.get('/', async (req, res) => {
    try {
        ...
        if (!ModularConfigService_1.modularConfigService.isLoaded()) {
            await ModularConfigService_1.modularConfigService.loadConfiguration();
        }
        const config = ModularConfigService_1.modularConfigService.getConfigurationSummary();
        res.json({
            success: true,
            data: config,
            timestamp: Date.now(),
        ...
        try {
            const legacyConfig = ConfigService_1.configService.getConfig();
            res.json({
                success: true,
                data: legacyConfig,
                timestamp: Date.now(),
...
router.get('/modules', async (req, res) => {
    try {
        ...
        const summary = ModularConfigService_1.modularConfigService.getConfigurationSummary();
        res.json({
            success: true,
            data: {
...
router.get('/modules/:moduleName', async (req, res) => {
    try {
        ...
        const moduleData = ModularConfigService_1.modularConfigService.getModuleData(moduleName);
        if (!moduleData) {
            return res.status(404).json({
                success: false,
                error: `Module not found: ${moduleName}`,
                availableModules: Object.keys(ModularConfigService_1.modularConfigService.getConfigurationSummary().masterConfig?.configuration_modules || {})
            });
        }
        res.json({
            success: true,
...
router.put('/modules/:moduleName', async (req, res) => {
    try {
        ...
        const success = await ModularConfigService_1.modularConfigService.saveModule(moduleName, data);
        if (success) {
            res.json({
                success: true,
            ...
            res.status(400).json({
                ...
                validationErrors: ModularConfigService_1.modularConfigService.getConfigurationSummary().validationErrors
            });
        }
    }
    catch (error) {
...
router.post('/validate', async (req, res) => {
    try {
        const validation = await ModularConfigService_1.modularConfigService.validateConfiguration();
        res.json({
            success: true,
            data: {
...
router.post('/reload', async (req, res) => {
    try {
        const config = await ModularConfigService_1.modularConfigService.reloadConfiguration();
        res.json({
            success: true,
            data: config,
            message: 'Configuration reloaded successfully',
...
router.get('/pins', async (req, res) => {
    try {
        ...
        const pins = ModularConfigService_1.modularConfigService.getAllPinConfigs();
        res.json({
            success: true,
            data: pins,
 ...
 */
router.get('/pins/:pinName', async (req, res) => {
    try {
        const { pinName } = req.params;
        const pinConfig = await ModularConfigService_1.modularConfigService.getPinConfig(pinName);
        if (!pinConfig) {
            return res.status(404).json({
                success: false,
                error: `Pin not found: ${pinName}`,
                availablePins: Object.keys(ModularConfigService_1.modularConfigService.getAllPinConfigs())
            });
        }
        res.json({
            success: true,
 ...
 */
router.get('/sonicators', async (req, res) => {
    try {
        if (!ModularConfigService_1.modularConfigService.isLoaded()) {
            await ModularConfigService_1.modularConfigService.loadConfiguration();
        }
        const sonicators = ModularConfigService_1.modularConfigService.getAllSonicatorConfigs();
        res.json({
            success: true,
            data: sonicators,
 ...
 */
router.get('/sonicators/:sonicatorId', async (req, res) => {
    try {
        ...
        const sonicatorConfig = await ModularConfigService_1.modularConfigService.getSonicatorConfig(sonicatorId);
        if (!sonicatorConfig) {
            return res.status(404).json({
                success: false,
 ...
 */
router.get('/db9', async (req, res) => {
    try {
        if (!ModularConfigService_1.modularConfigService.isLoaded()) {
            await ModularConfigService_1.modularConfigService.loadConfiguration();
        }
        const db9Configs = ModularConfigService_1.modularConfigService.getAllDB9Configs();
        res.json({
            success: true,
            data: db9Configs,
 ...
 */
router.get('/db9/:connectorName', async (req, res) => {
    try {
        const { connectorName } = req.params;
        const db9Config = await ModularConfigService_1.modularConfigService.getDB9Config(connectorName);
        if (!db9Config) {
            return res.status(404).json({
                success: false,
                error: `DB9 connector not found: ${connectorName}`,
                availableConnectors: Object.keys(ModularConfigService_1.modularConfigService.getAllDB9Configs())
            });
        }
        res.json({
            success: true,
...
router.get('/modbus', async (req, res) => {
    try {
        ...
        const modbusConfig = ModularConfigService_1.modularConfigService.getModbusConfig();
        if (!modbusConfig) {
            return res.status(404).json({
                success: false,
...
router.post('/export', async (req, res) => {
    try {
        ...
        const exportPath = await ModularConfigService_1.modularConfigService.exportCombinedConfig(outputPath);
        res.json({
            success: true,
            data: {
                exportPath,
