...
class MultiSonicatorSystem {
private:
    /**
     * @brief Global multi-sonicator status structure
     */
    multi_status_t g_multi = {
        MASTER_STATE_IDLE,
        0u,
    ...
    uint8_t g_start_inhibit_mask = 0u;      // per-unit inhibit for auto-promotion
    uint8_t g_start_timeout_ticks[4] = {0, 0, 0, 0};
...
public:
     ...
     */
    bool request_coordinated_start(uint8_t unit_mask) {
     ...
     * @return true if request was accepted, false otherwise
     */
    bool request_coordinated_stop(uint8_t unit_mask) {
        if (unit_mask == 0u) return false;
        if (g_multi.master_state == MASTER_STATE_EMERGENCY_STOP) return false;
     ...
     */
    bool request_unit_start(uint8_t unit_index) {
     ...
     */
    bool request_unit_stop(uint8_t unit_index) {
        if (unit_index > 3) return false;
     ...
     */
    bool report_unit_fault(uint8_t unit_index, bool overload) {
        if (unit_index > 3) return false;
    ...
    void set_start_inhibit(uint8_t unit_index, bool inhibit) {
    ...
    bool confirm_unit_started(uint8_t unit_index) {
        if (unit_index > 3) return false;
        if (g_multi.unit_state[unit_index] == SONICATOR_STATE_STARTING) {
     ...
     */
    const multi_status_t* get_status(void) {
        return &g_multi;
    }
};
