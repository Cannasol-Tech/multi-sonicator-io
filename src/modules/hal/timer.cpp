...
// Timer0 overflow interrupt (used for scheduler)
ISR(TIMER0_OVF_vect) {
    timer_states[TIMER_ID_0].interrupt_count++;
    timer_states[TIMER_ID_0].overflow_count++;
    
    // Check if this is the scheduler timer
    static uint8_t scheduler_prescaler_count = 0;
    scheduler_prescaler_count++;
    
    // Generate 10ms ticks (assuming 1ms timer interrupts)
    if (scheduler_prescaler_count >= 10) {
        scheduler_prescaler_count = 0;
        scheduler_ticks++;
